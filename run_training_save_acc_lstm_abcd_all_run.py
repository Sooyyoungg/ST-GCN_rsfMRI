...
train_data = np.load('data/train_data_all_run_1.npy')
train_label = np.load('data/train_label_all_run_1.npy')
test_data = np.load('data/test_data_all_run_1.npy')
test_label = np.load('data/test_label_all_run_1.npy')
...
for window_size in [50, 75, 100]: # [16, 32, 64, 128, 256, 512, 1200]:
    ...
    for fold in range(1, 6):
        ...
        train_data = np.load('data/train_data_all_run_' + str(fold) + '.npy')
        train_label = np.load('data/train_label_all_run_' + str(fold) + '.npy')
        test_data = np.load('data/test_data_all_run_' + str(fold) + '.npy')
        test_label = np.load('data/test_label_all_run_' + str(fold) + '.npy')
            ...
            if epoch % 1000 == 0:  # print every K mini-batches
                ...
                for v in range(TS):
                    ...
                    for k in range(int(test_data.shape[0] / batch_size)):
                        ...
                        test_data_batch = np.zeros((batch_size, 1, W, 48, 1))
                ...
                with open('output/mlp_baseline/all_run/testing_acc_folds/ws_' + str(W) + '_fold_' + str(fold) + '.txt', 'a') as f:
    ...
    with open('output/mlp_baseline/all_run/testing_acc_lstm_abcd_final.txt', 'a') as f:
