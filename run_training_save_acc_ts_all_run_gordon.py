...
train_data = np.load('data/gordon_train_data_all_run_1.npy')
train_label = np.load('data/gordon_train_label_all_run_1.npy')
test_data = np.load('data/gordon_test_data_all_run_1.npy')
test_label = np.load('data/gordon_test_label_all_run_1.npy')
...
for window_size in [25, 50, 64, 75, 100, 128, 256, 1200]: # [128]:
    W = window_size
    ...
    for fold in range(1, 6):
        ...
        train_data = np.load('data/gordon_train_data_all_run_' + str(fold) + '.npy')
        train_label = np.load('data/gordon_train_label_all_run_' + str(fold) + '.npy')
        test_data = np.load('data/gordon_test_data_all_run_' + str(fold) + '.npy')
        test_label = np.load('data/gordon_test_label_all_run_' + str(fold) + '.npy')
        ...
        for epoch in range(60001):  # number of mini-batches
            ...
            train_data_batch = np.zeros((batch_size, 1, W, 333, 1))
            ...
            # v:alidate on test subjects by voting
            if epoch % 1000 == 0:  # print every K mini-batches
                ...
                with open('output/all_run/testing_acc_st_gcn_folds_abcd/gordon/windowsize_' + str(W) + '_fold_' + str(fold) + '.txt', 'a') as f:
        ...
        print("Best accuracy for window {} and fold {} = {} at epoch = {}".format(W, fold, best_test_acc_curr_fold,
                                                                                  best_test_epoch_curr_fold))
        testing_acc_curr_fold.append(best_test_acc_curr_fold)
        layer_num = 1
        for edge_imp in best_edge_imp_curr_fold:
            filename = "output/all_run/edge_importance_abcd/gordon/edge_imp_layer_" + str(layer_num) + "_WS_" + str(W) + "_fold_" + str(fold)
    ...
    with open('output/all_run/testing_acc_st_gcn_folds_abcd/gordon_testing_acc_st_gcn_abcd_final.txt', 'a') as f:
