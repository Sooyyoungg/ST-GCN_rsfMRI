...
if __name__ == "__main__":
    ...
    L = 383 #timeseries
    S = 0
    data = np.zeros((demo.shape[0], 1, L, 333, 1));  # (289, 1, 383, 48, 1)
    ...
    for i in range(demo.shape[0]):
        ...
        filename_full = filename_dir + 'sub-' + subject_num + '_ses-baselineYear1Arm1_task-rest_run-1_space-MNIPediatricAsym_cohort-4_res-2_desc-preproc_atlas-gordon_timeseries.npy'
        full_sequence = np.load(filename_full).T;  # (333, 383)

        if full_sequence.shape[1] < S+L:
            continue
            # 여기서 길이가 383이 안되는 55개 날아가고 289개 중 234개 남음
    ...
    np.save('data/gordon_adj_matrix.npy', A)
    ...
    for train_idx, test_idx in skf.split(data, label):
        ...
        filename = 'data/gordon_train_data_'+str(fold)+'.npy'
        np.save(filename,train_data)
        filename = 'data/gordon_train_label_'+str(fold)+'.npy'
        np.save(filename,train_label)
        filename = 'data/gordon_test_data_'+str(fold)+'.npy'
        np.save(filename,test_data)
        filename = 'data/gordon_test_label_'+str(fold)+'.npy'
