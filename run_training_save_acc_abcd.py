...
for window_size in [25, 50, 64, 75, 100, 128, 256, 1200]: # [128]:
    ...
    for fold in range(1, 6):
        ...
        for epoch in range(60001):  # number of mini-batches
            ...
            if epoch % 1000 == 0:  # print every K mini-batches
                ...
                """
                if test_acc == 0.5531914893617021:
                    if all(prediction < 0.5):
                        print("all predictions are 0")"""

                with open('output/testing_acc_st_gcn_folds_abcd/gordon_windowsize_' + str(W) + '_fold_' + str(fold) + '.txt', 'a') as f:
        ...
        for edge_imp in best_edge_imp_curr_fold:
            filename = "output/edge_importance_abcd/gordon_edge_imp_layer_" + str(layer_num) + "_WS_" + str(W) + "_fold_" + str(fold)
    ...
    with open('output/gordon_testing_acc_st_gcn_abcd_final.txt', 'a') as f:
