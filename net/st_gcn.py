...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, num_class, graph_args,
        ...
        Dl = np.sum(A, 0) # number of adjacent nodes
        num_node = A.shape[0]
        Dn = np.zeros((num_node, num_node))
        for i in range(num_node):
            if Dl[i] > 0:
                Dn[i, i] = Dl[i] ** (-0.5)
        DAD = np.dot(np.dot(Dn, A), Dn) # matrix multiply Dn to get average.
        ...
        temporal_kernel_size = 11 # update temporal kernel size #논문과 동일
