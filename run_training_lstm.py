...
###### setup model & data
net = fMRI_LSTM(256, 48,  1, batch_size=batch_size)
net.to(device)
...
for epoch in range(100001):  # number of mini-batches
    ...
    if epoch % 1000 == 0:  # print every K mini-batches
        ...
        with open('output/mlp_baseline/testing_acc_lstm_abcd.txt', 'a') as f:
            f.write("Epoch {}, Test Acc {}\n".format(epoch, test_acc))

        if test_acc > best_test_acc_curr:
            best_test_acc_curr = test_acc
            best_test_acc_curr_epoch = epoch
            """tmp_edge_imp = []
            ...
            best_edge_imp_curr = tmp_edge_imp"""

        torch.save(net.state_dict(), 'checkpoint.pth')
        net.train()

print("At Epoch {}, Best Test Accuracy {}\n".format(best_test_acc_curr_epoch, best_test_acc_curr))
with open('output/mlp_baseline/testing_acc_lstm_abcd_final.txt', 'a') as f:
    f.write("At Epoch {}, Best Test Accuracy {}\n".format(best_test_acc_curr_epoch, best_test_acc_curr))
